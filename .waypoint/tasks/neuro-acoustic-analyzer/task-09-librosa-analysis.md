# Task 09: Implement Librosa Audio Analysis
...
Implement the core audio analysis using librosa. This extracts audio features (frequencies, tempo, energy, spectral features) that will be mapped to brain regions and emotions.

## Acceptance Criteria

- [ ] Load and process audio files
- [ ] Extract frequency spectrum (FFT)
- [ ] Detect tempo and beat positions
- [ ] Calculate energy/loudness per segment
...
`audio-service/src/analyzer/audio_processor.py`:
...
`audio-service/src/analyzer/__init__.py`:

```python
from .audio_processor import AudioProcessor, AudioFeatures, processor

__all__ = ['AudioProcessor', 'AudioFeatures', 'processor']
```
...
| File | Description |
|------|-------------|
| `audio-service/src/analyzer/audio_processor.py` | Audio analysis |
| `audio-service/src/analyzer/__init__.py` | Module export |

## Testing

```python
# test_processor.py
from src.analyzer import processor
...
for features in processor.process_audio("test_audio.mp3"):
    ...
    print(f"  Bass: {features.bass:.2f}")
    print(f"  Beat: {features.beat_strength:.2f}")
```

## Notes

- librosa is computationally intensive - consider caching
- Global tempo extracted once for consistency
- All values normalized to 0-1 for easy mapping
- Chunk duration of 1 second balances detail vs performance

---

_Task 09 of 28 - neuro-acoustic-analyzer_
